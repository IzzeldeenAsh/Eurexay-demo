...
@Component({
  selector: 'app-quarterly-payment',
  templateUrl: './quarterly-payment.component.html',
  styleUrls: ['./quarterly-payment.component.scss'],
  animations: [
    trigger('fadeInOut', [
...
export class QuarterlyPaymentComponent  implements OnInit {
  constructor(){
  ...
  totalBudget:number=0;
    ...
    initializePayments(){
      ...
      if (this.quarterlyPayments !== undefined) {
        for (let paymentNo = 1; paymentNo <= this.quarterlyPayments; paymentNo++) {
          this.payments.push({
            paymentNo: paymentNo,
            percentage: 0,
            amount: 0,
