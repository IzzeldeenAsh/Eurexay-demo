...
export class CurrentUserService {
  ...
  currentUserProfile = new BehaviorSubject<any>(null);
  currentUserProfile$ = this.currentUserProfile.asObservable();

  currentUserRole = new BehaviorSubject<any>('');
  currentUserRole$ = this.currentUserProfile.asObservable();


  getUserInfo():Observable<ICurrentUserResponse>{
    const dummyResponse: ICurrentUserResponse = {
      "status": {
          "code": "0",
          "description": "Success",
          "reason": "Success"
    ...
    this.currentUserName.next(dummyResponse.result.username);
    this.currentUserProfile.next(dummyResponse.result);
    this.currentUserRole.next(dummyResponse.result.participantType);
    // Simulate HTTP get request by returning an Observable of the dummy response
...
export function initializeApp(currentUserService: CurrentUserService) {
  return () => currentUserService.getUserInfo();
}

export function appInitializerFactory(currentUserService: CurrentUserService) {
  return () => {
    if (localStorage.getItem('token')) {
      return initializeApp(currentUserService)();
    } else {
